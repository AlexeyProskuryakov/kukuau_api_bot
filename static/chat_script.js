...
function showNotification(text, from){
    ...
    mailNotification.onclick = function(){
        console.log("trying to go to "+prefix);
    }
}
...
function paste_message(message){
    ...
    if (windowState != undefined && windowState == "blur" && message.From != _with){
...
$("#chat-form").on("submit", function(e){
    ...
    _from = $("#from").attr("value"),
    to = $("#with").attr("value");

    body = body.replace(/\?{2,}/g, "?");

    console.log("body: ", body, "from: ", _from, "to: ", to)
    $.ajax({
        type:           "POST",
        url:            url_prefix+"/send",
        data:           JSON.stringify({from:_from, to:to, body:body}),
        dataType:       'json',
        success:        function(x){
            console.log(x);
            if (x.ok == true) {
